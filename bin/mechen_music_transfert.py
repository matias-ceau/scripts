...
# INFO:#@RUN@~KJL~ "Custom script to copy music to Mechen"
...
# create df
df = pd.DataFrame(
    {"path": album_paths, "short_path": short_paths, "artist": artist, "size": sizes}
)

# get albums already on device
artists_on_device = [i for i in os.listdir(player_path) if os.path.isdir(i)]
albums_on_device = [
    [f"{i}/{b}" for b in os.listdir(os.path.join(player_path, i))]
...
for i in albums_on_device:
    if i not in df["short_path"].to_list():
        proc = subprocess.Popen(
            ["sudo", "-S", "rm", "-rf", os.path.join(player_path, i)],
            stdin=PIPE,
            stdout=PIPE,
            stderr=PIPE,
        )
...
for i in os.listdir(player_path):
    ...
    if os.path.isdir(p):
        if not os.listdir(p):
            proc = subprocess.Popen(
                ["sudo", "-S", "rm", "-rf", p], stdin=PIPE, stdout=PIPE, stderr=PIPE
            )
            proc.communicate(password.encode())
...
for i in df["short_path"].to_list():
    proc = subprocess.Popen(
        ["sudo", "-S", "mkdir", "-p", os.path.join(player_path, i)],
        stdin=PIPE,
        stdout=PIPE,
        stderr=PIPE,
    )
...
for n, i in enumerate(df["short_path"].to_list()):
    ...
    proc = subprocess.Popen(
        [
            ...
            "-rvu",
            "--delete",
            os.path.join("/home/matias/music", i)
            + "/",  # important for rsync to work properly
