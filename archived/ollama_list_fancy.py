...
parser.add_argument(
    ...
    "--popularity",
    action="store_true",
    help="Sort by popularity (number of downloads)",
)
parser.add_argument(
    "-n", "--newest", action="store_true", help="Sort by most recently updated"
)
parser.add_argument("-a", "--all", action="store_true", help="Show all models")
parser.add_argument(
    "-l", "--limit", type=int, default=10, help="Limit to N models (default: 5)"
)
parser.add_argument(
    "-d",
    "--describe",
    type=str,
    help="Show full description of the specified model name",
)
...
if response.status_code == 200:
    ...
    for container in model_containers:
        model_name_element = container.select_one(
            "a:nth-child(1) > h2:nth-child(1) > span:nth-child(1)"
        )
        model_description_element = container.select_one(
            "a:nth-child(1) > div:nth-child(2) > p:nth-child(1)"
        )
        downloads_element = container.select_one(
            "a:nth-child(1) > div:nth-child(2) > p:nth-child(3) > span:nth-child(1)"
        )
        last_updated_element = container.select_one(
            "a:nth-child(1) > div:nth-child(2) > p:nth-child(3) > span:nth-child(3)"
        )

        if (
            model_name_element
            and model_description_element
            and downloads_element
            and last_updated_element
        ):
            model_name = model_name_element.text.strip()
            model_description = model_description_element.text.strip()
            downloads, formatted_downloads = convert_downloads(
                downloads_element.text.strip()
            )
            last_updated, formatted_last_updated = convert_last_updated(
                last_updated_element.text.strip()
            )
            wrapped_description = "\n".join(
                textwrap.wrap(model_description, width=50)
    ...
    df = pd.DataFrame(
        model_list
    )  # .astype({'Model Name': 'str', 'Description': 'str', 'int_Downloads': 'float', 'int_Last Updated': 'float', 'Downloads' : 'str', 'Last Updated' : 'str'})
    ...
    if args.describe:
        described_model = df[df["Model Name"] == args.describe].drop(
            columns=["int_Downloads", "int_Last Updated"]
        )
        if not described_model.empty:
            table = tabulate(
                described_model, headers="keys", tablefmt="grid", showindex=False
            )
