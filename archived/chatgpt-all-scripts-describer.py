...
def find_source_file(binary_path):
    ...
    scripts_lib = os.path.join(os.environ.get('SCRIPTS', ''), 'lib')
...
def describe_script(script_content):
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a helpful assistant that generates GitHub documentation in markdown format for scripts."},
                {"role": "user", "content": f"Please generate the GitHub documentation (markdown format) of the following script (describe its functionalities):\n\n{script_content}"}
            ]
        )
...
def main():
    parser = argparse.ArgumentParser(description="Describe scripts from a CSV file using Llama 3.1 model.")
    parser.add_argument("csv_path", nargs='?', default=os.path.join(os.environ.get('SCRIPTS', ''), 'data', 'symlink_data.csv'),
